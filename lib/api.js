'use strict'

const Archetype = require('archetype-js')
const RegistrationType = require('./registration')
const { ObjectId } = require('mongodb')
const express = require('express')

module.exports = db => {
  const router = express.Router()

  const wrapAsync = handler => (req, res) => handler(req)
    .then(result => res.json(result))
    .catch(error => res.status(500).json({ error: error.message }))

  router.get('/', wrapAsync(async function(req) {
    return db.collection('Registrations').find().sort({ createdAt: -1 }).toArray()
  }))

  router.post('/', wrapAsync(async function(req) {
    const registration = new RegistrationType(req.body)
    await db.collection('Registrations').insertOne(registration)
    return { registration }
  }))

  router.delete('/:id', wrapAsync(async function(req) {
    const { result } = await db.collection('Registrations').deleteOne({
      _id: Archetype.to(req.params.id, ObjectId)
    })
    return { result }
  }))

  return router
}
